...
Use /buy to purchase a config or /test_config for a free test.`;
  ...
  static userConfigs(configs: any[]): string {
   ...
   Expires: ${expiresDate} (${remainingDays} days left)
   Data: ${dataUsed} GB used${dataLimit}
   Link: ${config.vless_link}\n\n`;
    });
...
Your payment will be processed once we verify the receipt.`;
  ...
  // No pending payment
  static noPendingPayment(): string {
    return `No pending payment found. Please start payment process with /add_funds`;
  }
