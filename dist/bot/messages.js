...
exports.BotMessages = void 0;
const v2ray_links_1 = require("../types/v2ray.links");
const telegram_format_1 = __importDefault(require("telegram-format"));
class BotMessages {
...
*Proceed with purchase\\?*`;
    ...
    // Insufficient funds
    static insufficientFunds(userBalance, servicePrice) {
...
You will receive a notification when it\\'s confirmed\\.`;
    ...
    static paymentConfirmedUser(amount) {
...
Thank you for your payment\\!`;
    ...
    static paymentConfirmedAdmin(payment) {
...
*Status:* CONFIRMED`;
    ...
    static paymentDeclinedUser(payment) {
...
Please contact support if you believe this is an error\\.`;
    ...
    static paymentDeclinedAdmin(payment) {
        const username = payment.user_id;
...
*Status:* DECLINED`;
    ...
    static accountInformation(user, configsCount) {
        const balance = Math.floor(user.balance);
...
📦 Use /buy to purchase configs`;
    ...
    // Support message
    static supportMessage(telegramId) {
        return `🛠️ *Support*
...
• Latest updates`;
    ...
    // Payment verification required (admin)
    static paymentVerificationRequired(payment, user) {
        const username = user.username ? this.escapeMarkdown(user.username) : 'N/A';
...
*Card:* ${this.escapeMarkdown(payment.card_number)}`;
    ...
    static purchaseSuccessful(service, links) {
        return v2ray_links_1.VlessLinkGenerator.formatForDisplay(links) + '\n\n' +
            `🎉 *Purchase Successful\\!*\n\n` +
            `*Service:* ${this.escapeMarkdown(service.name)}\n` +
            `*Expires:* In ${service.duration_days} days\n\n` +
            `📋 Use /my\\_services to view all your configs\\.`;
    ...
    static getPlatformLinkMessage(links, platform) {
        let platformLink = '';
        let platformName = '';
        ...
        if (activeServices.length > 0) {
            ...
            activeServices.forEach((service, index) => {
                ...
                const dataLimit = service.data_limit_gb
                    ...
                    : 'Unlimited';
                const remainingDays = Math.ceil((new Date(service.expires_at).getTime() - Date.now()) / (1000 * 60 * 60 * 24));
    ...
    static userServicesDetailed(services) {
        if (services.length === 0) {
            return this.noActiveConfigs();
        }
        let message = telegram_format_1.default.markdown.bold('📋 Detailed Service Information\n\n');
        services.forEach((service, index) => {
            ...
            const dataLimit = service.data_limit_gb
                ? `${service.data_limit_gb} GB`
                : 'Unlimited';
            message += `${index + 1}. ${telegram_format_1.default.markdown.bold('Service Information')}\n`;
            message += `   ${telegram_format_1.default.markdown.bold('Service ID:')} ${service.config_id}\n`;
            message += `   ${telegram_format_1.default.markdown.bold('Name:')} ${service.service_name || 'N/A'}\n`;
            message += `   ${telegram_format_1.default.markdown.bold('Status:')} ${this.getStatusEmoji(service.status)} ${service.status}\n`;
            message += `   ${telegram_format_1.default.markdown.bold('Created:')} ${createdDate}\n`;
            message += `   ${telegram_format_1.default.markdown.bold('Last Updated:')} ${updatedDate}\n`;
            message += `   ${telegram_format_1.default.markdown.bold('Expires:')} ${expiresDate} (${remainingDays > 0 ? `${remainingDays} days left` : 'Expired'})\n`;
            message += `   ${telegram_format_1.default.markdown.bold('Data Used:')} ${dataUsed} GB / ${dataLimit}\n`;
            if (service.service_price) {
                message += `   ${telegram_format_1.default.markdown.bold('Original Price:')} $${service.service_price}\n`;
            }
            if (service.service_duration) {
                message += `   ${telegram_format_1.default.markdown.bold('Original Duration:')} ${service.service_duration} days\n`;
            }
            message += '\n';
        });
        message += telegram_format_1.default.markdown.bold('📝 Support Information:\n');
...
exports.BotMessages = BotMessages;
// Callback query answers
BotMessages.callbackAnswers = {
